...
>
> 关于 `codex-ppt` 和 `editable-ppt` 这两个技能的详细介绍，参见 [skill_duo_intro.pdf](assets/skill_duo_intro.pdf)。该 PPT 由 `codex-ppt` skill 生成，提示词为：“请分别阅读 Codex PPT和 Editable PPT 这两个技能的内容，然后用 Codex PPT 帮我做一个PPT吧，20页，每个技能的介绍10页。”
>
...
```text
安装 editable-ppt 这个 skill，地址是 https://github.com/ningzimu/image-to-editable-ppt-skill
```
...
最简单的方式是在 Codex 对话里直接让 AI 安装这个 skill：
...
安装完成后，在 Codex 中直接这样调用：

```text
$editable-ppt 把这张图片转成可编辑 PPT。
$editable-ppt 把这个 PDF 转成可编辑 PPT。
$editable-ppt 把这个图片版 PPTX 转成可编辑 PPT。
```

## 更新

```text
更新 editable-ppt 这个 skill，地址是 https://github.com/ningzimu/image-to-editable-ppt-skill
```

## 使用方式

在支持显式选择 skill 的 agent 里，可以用对应语法选中 `editable-ppt`；Codex 中可使用 `$editable-ppt`。图片、PDF 和 `.pptx` 可以直接粘贴或附加到对话框，也可以提供本地路径：

```text
$editable-ppt 把这张图片转成可编辑 PPT。
$editable-ppt 把这些图片转成一个可编辑 PPT。
$editable-ppt 把 <path-to-deck.pdf> 转成可编辑 PPT。
$editable-ppt 把 <path-to-image-based.pptx> 转成可编辑 PPT。
```
...
```text
output/editable-ppt/{job-id}/                 # 单次转换任务目录
├── input/                                    # 原始输入文件副本
...
├── skills/                               # Skill 安装包目录
│   └── editable-ppt/                     # 可安装的 editable-ppt skill
...
<img src="assets/image-to-editable-ppt-community-qr.png" alt="Editable PPT Skill 交流群二维码" width="220">
