...
This repository packages the `editable-ppt` skill.

- Repository-level docs, examples, CI, and release metadata live at the repository root.
- The installable skill lives under `skills/editable-ppt/`.
...
- Do not edit files under `skills/editable-ppt/` unless the task explicitly asks to change the skill itself.
...
The skill documentation under `skills/editable-ppt/` follows a strict ownership model. Future edits must preserve it.
...
- The release workflow uploads `image-to-editable-ppt-skill-v*.zip`, which contains only the installable `editable-ppt` skill directory.
