...
>
> For a detailed introduction to `codex-ppt` and `editable-ppt`, see [skill_duo_intro.pdf](assets/skill_duo_intro.pdf). This deck was generated with the `codex-ppt` skill using the prompt: "请分别阅读 Codex PPT和 Editable PPT 这两个技能的内容，然后用 Codex PPT 帮我做一个PPT吧，20页，每个技能的介绍10页。"
>
...
```text
Install the editable-ppt skill from https://github.com/ningzimu/image-to-editable-ppt-skill
```
...
The easiest path is to ask Codex to install the skill in chat:
...
npx -y skills@latest add ningzimu/image-to-editable-ppt-skill \
  ...
  --agent <agent-id> \
  --global
pipx install --force --editable <path-to-repo>/skills/editable-ppt/cli
editppt doctor
```
...
```text
$editable-ppt convert this image into an editable PowerPoint.
$editable-ppt convert this PDF into an editable PowerPoint.
$editable-ppt convert this image-based PPTX into an editable PowerPoint.
```

## Update

```text
Update the editable-ppt skill from https://github.com/ningzimu/image-to-editable-ppt-skill
```

## Usage

In agents that support explicit skill selection, use the agent's syntax to select `editable-ppt`; in Codex, use `$editable-ppt`. Images, PDFs, and `.pptx` files can be pasted or attached directly in the conversation, or provided as local paths:

```text
$editable-ppt convert this image into an editable PowerPoint.
$editable-ppt convert these images into one editable PowerPoint.
$editable-ppt convert <path-to-deck.pdf> into an editable PowerPoint.
$editable-ppt convert <path-to-image-based.pptx> into an editable PowerPoint.
```
...
```text
output/editable-ppt/{job-id}/                 # One conversion job folder
...
│   └── editable-ppt/                     # Installable editable-ppt skill
...
<img src="assets/image-to-editable-ppt-community-qr.png" alt="Editable PPT Skill community QR code" width="220">
