...
TEMPERATURE = (
    float(os.environ["AZURE_OPENAI_TEMPERATURE"])
    if os.environ.get("AZURE_OPENAI_TEMPERATURE")
    else None
)
OPTIMIZER_TEMPERATURE = (
    float(os.environ["OPTIMIZER_AZURE_OPENAI_TEMPERATURE"])
    if os.environ.get("OPTIMIZER_AZURE_OPENAI_TEMPERATURE")
    else TEMPERATURE
)
TARGET_TEMPERATURE = (
    float(os.environ["TARGET_AZURE_OPENAI_TEMPERATURE"])
    if os.environ.get("TARGET_AZURE_OPENAI_TEMPERATURE")
    else TEMPERATURE
)
...
def _make_client(role: str) -> AzureOpenAI | OpenAI:
    ...
    if auth_mode in {"openai_compatible", "compat", "openai"}:
        ...
        if cfg["api_key"]:
            # Some enterprise OpenAI-compatible gateways require x-api-key
            # instead of relying solely on the Authorization bearer header.
            default_headers["x-api-key"] = cfg["api_key"]
        return OpenAI(
...
def configure_azure_openai(
    *,
    ...
    optimizer_temperature: float | None = None,
    ...
    global TEMPERATURE, OPTIMIZER_TEMPERATURE, TARGET_TEMPERATURE
    ...
    def _clean_float(value: float | str | None) -> float | None:
        if value is None:
            return None
    ...
    shared_temperature = _clean_float(temperature)
    ...
    resolved_optimizer_temperature = _clean_float(optimizer_temperature)
    if resolved_optimizer_temperature is None:
        resolved_optimizer_temperature = shared_temperature
    ...
    resolved_target_temperature = _clean_float(target_temperature)
    if resolved_target_temperature is None:
        resolved_target_temperature = shared_temperature
    ...
    OPTIMIZER_TEMPERATURE = resolved_optimizer_temperature
    TARGET_TEMPERATURE = resolved_target_temperature
    if shared_temperature is None:
        os.environ.pop("AZURE_OPENAI_TEMPERATURE", None)
    else:
        os.environ["AZURE_OPENAI_TEMPERATURE"] = str(shared_temperature)
    if resolved_optimizer_temperature is None:
        os.environ.pop("OPTIMIZER_AZURE_OPENAI_TEMPERATURE", None)
    else:
        os.environ["OPTIMIZER_AZURE_OPENAI_TEMPERATURE"] = str(resolved_optimizer_temperature)
    if resolved_target_temperature is None:
        os.environ.pop("TARGET_AZURE_OPENAI_TEMPERATURE", None)
    else:
        os.environ["TARGET_AZURE_OPENAI_TEMPERATURE"] = str(resolved_target_temperature)
...
def chat_optimizer(
    ...
    return _chat_impl(
        ...
        system, user, max_completion_tokens, retries, stage, reasoning_effort, OPTIMIZER_TEMPERATURE, timeout,
    )
...
def chat_target(
    ...
    return _chat_impl(
        ...
        system, user, max_completion_tokens, retries, stage, reasoning_effort, TARGET_TEMPERATURE, timeout,
    )
