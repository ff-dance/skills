...
def _rollout_eval_repeats(
    *,
    ...
    out_root: str,
    repeats: int,
) -> tuple[float, float, int, list[dict], dict]:
    """Run repeated eval rollouts and aggregate mean hard/soft scores."""
    repeat_count = max(int(repeats or 1), 1)
    ...
    for idx in range(repeat_count):
        eval_env, eval_n = build_eval_env(
            split=split,
            env_num=env_num,
            seed=seed,
        )
        n_items = eval_n
        repeat_dir = out_root if repeat_count == 1 else os.path.join(out_root, f"repeat_{idx + 1:02d}")
        repeat_results = adapter.rollout(eval_env, skill_content, repeat_dir)
        rep_hard, rep_soft = compute_score(repeat_results)
        hard_scores.append(rep_hard)
        soft_scores.append(rep_soft)
        combined_results.extend(repeat_results)
    ...
    if repeat_count > 1:
        with open(os.path.join(out_root, "aggregate_summary.json"), "w") as f:
            json.dump(aggregate, f, indent=2, ensure_ascii=False)

    return (
        float(aggregate["hard_mean"]),
        float(aggregate["soft_mean"]),
        n_items,
        combined_results,
        aggregate,
    )
...
class ReflACTTrainer:
    ...
    def train(self) -> dict:
        ...
        configure_azure_openai(
            ...
            temperature=cfg.get("azure_openai_temperature"),
            ...
            optimizer_temperature=cfg.get("optimizer_azure_openai_temperature"),
            ...
            target_temperature=cfg.get("target_azure_openai_temperature"),
        ...
        selection_repeats = max(int(cfg.get("selection_repeats", 1) or 1), 1)
        test_repeats = max(int(cfg.get("test_repeats", 1) or 1), 1)
        print(
        ...
        if selection_repeats > 1 or test_repeats > 1:
            print(
                f"  [eval repeats] selection={selection_repeats} test={test_repeats}"
            )
        ...
        if current_score < 0:
            ...
            baseline_hard, baseline_soft, _, _, baseline_agg = _rollout_eval_repeats(
                adapter=adapter,
            ...
            if selection_repeats > 1:
                print(
                    f"  [baseline repeats] hard={baseline_agg['hard_scores']} "
                    f"soft={baseline_agg['soft_scores']}"
                )
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step_in_epoch in range(steps_per_epoch):
                ...
                if cand_hash in sel_cache:
                    ...
                    cand_hard, cand_soft, _, _, sel_agg = _rollout_eval_repeats(
                        adapter=adapter,
                        build_eval_env=_build_eval_env,
                        split="valid_seen",
                        env_num=cfg["sel_env_num"],
                        ...
                        out_root=sel_eval_dir,
                        repeats=selection_repeats,
                    )
                    sel_cache[cand_hash] = (cand_hard, cand_soft)
                    if selection_repeats > 1:
                        print(
                            f"    [6/6 repeats] hard={sel_agg['hard_scores']} "
                            f"soft={sel_agg['soft_scores']}"
                        )
        ...
        if cfg["eval_test"]:
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    final_selection_hard, final_selection_soft, _, _, fval_agg = _rollout_eval_repeats(
                        adapter=adapter,
                        build_eval_env=_build_eval_env,
                        split="valid_seen",
                        env_num=cfg["sel_env_num"],
                    ...
                    if selection_repeats > 1:
                        print(
                            f"  [final val repeats] hard={fval_agg['hard_scores']} "
                            f"soft={fval_agg['soft_scores']}"
                        )
            ...
            baseline_test_hard, baseline_test_soft, _, baseline_test_results, baseline_test_agg = _rollout_eval_repeats(
                adapter=adapter,
                build_eval_env=_build_eval_env,
                split="valid_unseen",
                env_num=cfg["test_env_num"],
            ...
            if test_repeats > 1:
                print(
                    f"  [baseline test repeats] hard={baseline_test_agg['hard_scores']} "
                    f"soft={baseline_test_agg['soft_scores']}"
                )
            ...
            test_hard, test_soft, _, test_results, test_agg = _rollout_eval_repeats(
                adapter=adapter,
                build_eval_env=_build_eval_env,
                split="valid_unseen",
                env_num=cfg["test_env_num"],
            ...
            if test_repeats > 1:
                print(
                    f"  [best test repeats] hard={test_agg['hard_scores']} "
                    f"soft={test_agg['soft_scores']}"
                )
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    final_test_dir = os.path.join(out_root, "test_eval_final")
                    final_test_hard, final_test_soft, _, final_test_results, final_test_agg = _rollout_eval_repeats(
                        adapter=adapter,
                        build_eval_env=_build_eval_env,
                        split="valid_unseen",
                        env_num=cfg["test_env_num"],
                    ...
                    if test_repeats > 1:
                        print(
                            f"  [final test repeats] hard={final_test_agg['hard_scores']} "
                            f"soft={final_test_agg['soft_scores']}"
                        )
                    with open(os.path.join(final_test_dir, "summary.json"), "w") as f:
                        json.dump(
                            {
